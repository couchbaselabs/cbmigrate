	...
	Describe("generate partial filter expression", func() {
		Context("success", func() {
			...
			It("generate partial filter expression with type", func() {
				...
				output := "WHERE (type(`a`) = \"number\" AND type(meta().id) = \"string\")"
				result, err := mongo.ConvertMongoToCouchbase(partialFilter, fieldPath)
				Expect(err).To(BeNil())
				if result != output {
					fmt.Println("\n" + result)
			...
			It("generate partial filter expression with type array", func() {
				partialFilter := bson.D{
					...
					},
					{
						Key: "b", Value: bson.D{
							{
								Key: "$type", Value: bson.A{"string", "object", int32(4), float64(5.000)},
							},
						},
					},
				}
				...
				output := "WHERE (type(`a`) = \"number\" AND type(`b`) IN [\"string\",\"object\",\"array\",\"binary\"])"
				result, err := mongo.ConvertMongoToCouchbase(partialFilter, fieldPath)
				Expect(err).To(BeNil())
				if result != output {
					fmt.Println("\n" + result)
